  ...
  <div class="accordion-item">
    ...
    <div id="tvetActivities" class="accordion-collapse collapse" aria-labelledby="headingThree" data-bs-parent="#accordionTvet">
     ...
     <div class="mb-4 pt-5 d-flex">
            <span  class="text-success" :class="{'text-danger':false}">{{}}</span>
            <button @click="back"  class="btn ms-auto border px-3">Back</button>
            <button @click="saveChanges"  type="button"  class="btn btn-add text-white mx-3">
            <span v-if="isSaving">
              <span   class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                Saving
              </span>      
            <span v-else class="text-white">Save Changes</span>
          </button>
      </div>
  </div>
  ...
  <vue-modal :modalState="modalState">
     <div class="modal-content mt-4 mx-auto bg-white p-3">
        ...
        <div class="body ">
         ...
         <div class="mb-3" :class="{warining:v$.plan.activity.$error}"> 
              <label for="#planactivity" class="form-label">Activity</label>
              <textarea class="form-control " v-model.trim="plan.activity" @blur="v$.plan.activity.$touch" id="planactivity" rows="5" aria-label=".form-control"></textarea>
              <span class="error-msg mt-1"  v-for="(error, index) of v$.plan.activity.$errors" :key="index">{{ error.$message+", " }}</span>
         </div> 
        </div>
         <div class="footer">
...
export default {
  ...
  data(){
    ...
    computed:{
    ...mapGetters({academicCalendarDetail:'dean/selectedAcademicCalendarDetail', slectedYearId:'selectedAcademicYearId'})
  },
   methods:{
    async saveChanges(){
           this.isSaving=true
            let temp={
                 academic_year_id:this.slectedYearId,
                  semesters:[
                    {
                      semester_id:this.academicCalendarDetail.regular?.[0]?.id,
                      activities:[...this.activities.regular.firstSemester]
                    },
                     {
                      semester_id:this.academicCalendarDetail.regular?.[1].id,
                      activities:[...this.activities.regular.secondSemester]
                    },
                     {
                      semester_id:this.academicCalendarDetail.extension?.[0]?.id,
                      activities:[...this.activities.extension.firstSemester]
                    },
                     {
                      semester_id:this.academicCalendarDetail.extension?.[1]?.id,
                      activities:[...this.activities.extension.secondSemester]
                    },
                     {
                      semester_id:this.academicCalendarDetail.extension?.[2]?.id,
                      activities:[...this.activities.extension.thirdSemester]
                    }
                  ],
                  tvet_activities:[...this.activities.tvet]
                 }
         try{
    ...
    addActivity(){
      ...
      setFields(){
      this.activities.regular.firstSemester=[...this.academicCalendarDetail.regular?.[0]?.degree_calender_activities]
      this.activities.regular.secondSemester=this.academicCalendarDetail.regular?.[1]?.degree_calender_activities
      this.activities.extension.firstSemester=this.academicCalendarDetail.extension?.[0]?.degree_calender_activities
      this.activities.extension.secondSemester=this.academicCalendarDetail.extension?.[1]?.degree_calender_activities
      this.activities.extension.thirdSemester=this.academicCalendarDetail.extension?.[2]?.degree_calender_activities

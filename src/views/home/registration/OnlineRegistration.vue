<template>
  <base-card>
    ...
    <div class="d-flex mt-1 mt-md-3 container">
      <div
        @click="personal()"
        class="personal pointer me-3 flex-fill"
      ...
      <div
        @click="educational()"
        class="educational pointer flex-fill"
        :class="{ activePointer: isEducational, endPointer: isEducational }"
      >
        Educational Info
      </div>
    </div>
...
export default {
  ...
  created() {
    this.$store.dispatch("fetchAcademicYears").then(() => {
    ...
    async registerNewDegreeStudent() {
      ...
      try {
        ...
        var response = await apiClient.post(
          "api/degree_online_registration",
          this.studentInfo
        );

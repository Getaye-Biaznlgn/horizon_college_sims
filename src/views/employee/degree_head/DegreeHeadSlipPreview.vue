<template>
  <base-card class="px-3 mx-4 mt-3">
    ...
    <div v-else class="mt-2">
      Faild to preview Students slip. Select Students and try again please!
    </div>
    <div id="slip">
      <div
        v-for="slip in studentForSlip"
        :key="slip.id"
        ...
        <div v-for="n in 2" :key="n">
          ...
          <div class="d-flex justify-content-between">
            <div>
              ...
              <div>
                Full Name: <u>{{ slip.first_name + " " + slip.last_name }}</u>
              </div>
            </div>
            <div>
          ...
          <div>
            <span class="pe-2"
              >Department: <u>{{ user.manage?.name }}</u></span
            >
            <span class="pe-2"
              >Academic Year: <u>{{ getYearById.year }}</u></span
            >
            <span class="pe-2"
              >Year: <u>{{ year_no }}</u></span
            >
            <span class="pe-2"
              >Semester: <u>{{ semester_no }}</u></span
            >
          </div>
          <table class="mt-3">
            <tbody>
          ...
          <div class="d-flex ">
            <div class="ms-auto border px-2">
              ...
              Total Fee: <u>Birr {{ cpFee * totalCP + registrationFee }}</u>
              <br />
            </div>
          </div>

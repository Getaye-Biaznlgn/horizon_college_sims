<template>
  <base-card class="px-3 mx-4 mt-3">
    <span @click="back" class="back pe-2 fw-bold" role="button"
      ><i class="fas  fa-arrow-left"></i>Back</span
    >
    <div class="d-flex" v-if="studentCourses.length">
      <button
        @click="printGradeReport"
        class="btn btn-add ms-auto text-white me-2 mb-3 shadow-sm"
      >
        <i class="fas fa-print me-2"></i>Print Grade Report
      </button>
    </div>
    <div v-else class="mt-2">
      Faild to preview Students grade report. Select Students and try again
      please!
    </div>

    <div id="slip">
      <div
        v-for="student in studentCourses"
        :key="student.id"
        style="height:260mm; overflow-y:hidden;"
      >
        ...
        <div class="basic-info">
          <div class="d-flex">
            <span class=""
              >Full Name:
              <u>{{ student.first_name + " " + student.last_name }}</u></span
            >
            <span class="ms-auto"
              >Sex: {{ student.sex?.toString().substring(0, 1) }}
            </span>
            <span class="ms-auto"
              >ID No: <u>{{ student.student_id }}</u></span
            >
            <span class="ms-auto"
              >Admission Classification: <u>{{ student.program }}</u>
            </span>
          </div>
        </div>
        <div class="d-flex mt-1">
          <span class="pe-2"
            >Department: <u>{{ student.department_name }}</u></span
          >
          <span class=" ms-auto"
            >Date of Admission: <u>{{ student.addmission_year }}</u></span
          >
        ...
        <table class="my-2">
          ...
          <tr>
            <th class="text-center" colspan="6">
              Year {{ year_no }} Semester {{ semester_no }} A Year:
            </th>
          </tr>
          <tbody>
        ...
        <div>
          ...
          Comulative Average: {{ student.CGPA?.toString().substring(0, 4)
          }}<br />
        </div>
      </div>
    </div>

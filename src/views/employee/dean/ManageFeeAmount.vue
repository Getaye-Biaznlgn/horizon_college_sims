          ...
          <td v-show="!isPrinting">
            <div class="dropdown">
              ...
              <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                <li>
                  <span @click="showEditModal(fee)" class="dropdown-item"
                    >Edit</span
                  >
                </li>
              </ul>
    ...
    <template #modalBody>
      <form @submit.prevent>
        <div class="mb-3" :class="{ warining: v$.fee.name.$error }">
          ...
          <input
            :disabled="actionButtonType === 'edit'"
            class="form-control "
            v-model.trim="fee.name"
            @blur="v$.fee.name.$touch"
            id="feename"
            name="text"
            placeholder="Eg. name"
          ...
          <span
            ...
            v-for="(error, index) of v$.fee.name.$errors"
            :key="index"
            >{{ error.$message + ", " }}</span
          >
        </div>
        ...
        <div class="mb-3" :class="{ warining: v$.fee.amount.$error }">
          ...
          <input
            class="form-control "
            v-model.trim="fee.amount"
            @blur="v$.fee.amount.$touch"
            id="feeAmount"
            type="number"
          ...
          <span
            class="error-msg mt-1"
            v-for="(error, index) of v$.fee.amount.$errors"
            :key="index"
            >{{ error.$message + ", " }}</span
          >
        </div>
      </form>
      <request-status-notifier
        :notificationMessage="responseMessage"
        :isNotSucceed="isNotSucceed"
      ></request-status-notifier>
    </template>
  </base-modal>
</template>
  ...
  methods: {
    ...
    async fetchFeeAmount(yearId) {
      ...
      try {
        var response = await apiClient.get(
          "/api/get_academic_fees?academic_year_id=" + yearId
        );
    ...
    async edit() {
      ...
      if (!this.v$.$error) {
        this.isSaving = true;
        await apiClient
          .post("/api/set_academic_fees/" + this.selectedYearId, this.fee)
          .then(response => {
            if (response.status === 200) {

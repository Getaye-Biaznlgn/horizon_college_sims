<template>
  <div class="border rounded shadow-sm m-3 bg-white">
    <div>
      <span @click="$router.back()" class="backarrow ms-3 mt-2"
    ...
    <div class="d-flex justify-content-between align-items-center">
      ...
      <div class="d-flex justify-content-end mt-1">
        ...
        <button @click="addStudentToCoc()" class="btn me-1 addbtn me-3 p-1">
          Add Student
        </button>
    ...
    <div id="coctakers" class="mt-3">
      <table class="viewcourse courseview mt-2">
        ...
        <tbody>
          <tr v-for="(student, index) in filteredStudnt" :key="student.id">
            <td class="px-3">
              {{
                queryObject.per_page * cocTakerStudents.current_page +
                  index +
                  1 -
            ...
            <td>
             ...
             <hr class="my-0 py-0"> -->
                  <li>
                    <span
                      @click="enterResult(student.student_id, student.type)"
                      class="dropdown-item"
                      >Enter Result
                    </span>
                  </li>
                  ...
                  <li>
                    <span
                      @click="
                        enterCerteficateNo(student.student_id, student.type)
                      "
                      class="dropdown-item"
                      >Enter Certeficat Number
                    </span>
                  </li>
                  <hr class="my-0 py-0" />
                  <li>
      ...
      <div v-if="!filteredStudnt?.length" class="p-2 ms-5 px-5">
        There is no students registered for this COC exam
      </div>
    </div>
    <div
      v-if="filteredStudnt?.length"
    ...
    >
      ...
      <div class="pageno me-3">
        {{
          cocTakerStudents.from +
            "-" +
            cocTakerStudents.to +
            " of " +
            cocTakerStudents.total +
            " pages"
        }}
      </div>
      <div class="leftchivron ms-3 me-3">
        <button
          @click="backChivron()"
          class="chivronbtn"
          :class="{ active: cocTakerStudents.from !== 1 }"
          :disabled="cocTakerStudents.from === 1"
        >
      ...
      <div class="rightchivron">
        <button
          ...
          class="chivronbtn"
          :class="{ active: cocTakerStudents.to !== cocTakerStudents.total }"
          :disabled="cocTakerStudents.to === cocTakerStudents.total"
        >
  ...
  <div v-if="isEnterResult" class="editwraper border shadow-sm">
    <div class="w-50 ms-auto content me-auto mt-5 p-5 bg-white">
      <div
        ...
        :class="{ warning: v$.inputValue.$error || Number(inputValue) > 100 }"
      >
        ...
        <span v-if="Number(inputValue) > 100" class="error-msg mt-1"
          >value must be lessthan or equal to 100</span
        >
      </div>
      <p
  ...
  <div v-if="isEnterCerteficateNo" class="editwraper border shadow-sm">
    <div class="w-50 h-50 ms-auto content me-auto mt-5 p-5 bg-white">
      <div
        class="form-group"
        :class="{ warning: v$.inputValue.$error || inputValue.length > 15 }"
      >
        ...
        <span v-if="inputValue?.length > 15" class="error-msg mt-1"
          >value must be lessthan or equal to 15 digits</span
        >
      </div>
      <p
        class="ms-5 mt-3 text-center"
...
export default {
  ...
  validations() {
    return {
      inputValue: {
        required: helpers.withMessage("Please Enter value", required)
      }
    };
  },
  computed: {
  ...
  methods: {
    ...
    yesDelete() {
      ...
      this.$store.dispatch(
        "registrar/deleteStudentFromCoc",
        this.deletedStudent
      );
      this.isDelete = false;
    },
      ...
      if (!this.v$.$error && Number(this.inputValue) <= 100) {
        ...
        try {
          var response = await apiClient.post(
            "api/give_certeficate_no/" + data.id,
            data
          );
          if (response.status === 200) {
            ...
            var previousStudents = this.$store.getters[

<template>
  <div class="content border rounded shadow-sm mt-2 w-75 ms-auto me-auto mb-5">
    <div class="ms-5 pt-3">
      <span @click="back()" class="backarrow me-5"
    ...
    <div class="feetype row ms-5 mt-3">
      <button
        @click="internalStudent()"
        class="btn optionbtn col-sm-6 me-0 p-2 fs-5"
        :class="{ internal: isInternal }"
      >
        Internal Students
      </button>
      <button
        @click="externalStudent()"
        class="btn optionbtn col-sm-5 ms-0 p-2 fs-5"
    ...
    <div v-if="isInternal" class="mt-5">
      ...
      <div class="mb-3 mt-3 px-5" :class="{ warning: isIdEmpty || idError }">
        ...
        <input
          ...
          id="id"
          v-model="studentId"
          @input="checkIdLength($event)"
        />
        <span v-if="isIdEmpty" class="error-msg mt-1"
          >Student ID number is required
        </span>
        <span v-if="idError" class="error-msg mt-1"
          >Student ID number must be 12 digit long
        </span>
      </div>
      ...
      <div
        class="mb-3 mt-3 px-5"
        :class="{ warning: isOccupationEmpty || occupationError }"
      >
        <label for="Ocupation" class="form-label"
          ><strong>Ocupation</strong></label
        >
        <input
          type="text"
          class="form-control"
          ...
          @input="checkOccupationLength($event)"
        />
        <span v-if="isOccupationEmpty" class="error-msg mt-1"
          >Occupation field can not be empty</span
        >
        <span v-if="occupationError" class="error-msg mt-1"
          >The maximum character length of Occupation Name is 25 characters long
        </span>
      </div>
      <div
        class="mb-3 px-5"
        :class="{ warning: isRegistrationEmpty || registrationNoError }"
      >
        <label for="Registration" class="form-label"
          ><strong>Registration Number</strong></label
        >
        <input
          type="text"
          class="form-control"
          ...
          @input="checkRegistrationLength($event)"
        />
        <span v-if="isRegistrationEmpty" class="error-msg mt-1"
          >Registration number is required
        </span>
        <span v-if="registrationNoError" class="error-msg mt-1"
          >The maximum length of Registration number is 15 digits long</span
        >
      </div>
    ...
    <div v-if="isExternal">
      <div
        class="mb-3 mt-5 px-5"
        :class="{ warning: v$.cocStudent.first_name.$error }"
      >
        ...
        <input
          ...
          class="form-control"
          id="fname"
          v-model="cocStudent.first_name"
        />
        <span class="error-msg mt-1">{{
          v$.cocStudent.first_name.$errors[0]?.$message
        }}</span>
      </div>
      <div
        class="mb-3 px-5"
        :class="{ warning: v$.cocStudent.last_name.$error }"
      >
        ...
        <input
          ...
          class="form-control"
          id="lname"
          v-model="cocStudent.last_name"
        />
        <span class="error-msg mt-1">{{
          v$.cocStudent.last_name.$errors[0]?.$message
        }}</span>
      </div>
      ...
      <div class="d-flex mt-4 px-5">
        ...
        <div class="form-check me-4">
          <input
            ...
            id="Single"
            value="single"
            v-model="cocStudent.maritial_status"
          />
      ...
      <div
        class="mb-3 mt-3 px-5"
        :class="{ warning: v$.cocStudent.dob.$error }"
      >
        ...
        <input
          ...
          class="form-control"
          id="dob"
          v-model="cocStudent.dob"
        />
        <span class="error-msg mt-1">{{
          v$.cocStudent.dob.$errors[0]?.$message
        }}</span>
      </div>
      <div
        class="mb-3 mt-3 px-5"
        :class="{ warning: v$.cocStudent.phone_no.$error }"
      >
        ...
        <input
          ...
          class="form-control"
          id="phone"
          v-model="cocStudent.phone_no"
        />
        <span class="error-msg mt-1">{{
          v$.cocStudent.phone_no.$errors[0]?.$message
        }}</span>
      </div>
      <div class="ms-auto me-auto mt-3 px-5">
        <div
          class="mb-3 mt-3"
          :class="{ warning: v$.cocStudent.occupation_name.$error }"
        >
          <label for="Ocupation" class="form-label"
            ><strong>Ocupation</strong></label
          >
          <input
            ...
            class="form-control"
            id="Ocupation"
            v-model="cocStudent.occupation_name"
          />
          <span class="error-msg mt-1">{{
            v$.cocStudent.occupation_name.$errors[0]?.$message
          }}</span>
        </div>
        <div class="mt-3">
        ...
        <div
          class="mb-3 mt-3"
          :class="{ warning: v$.cocStudent.registration_no.$error }"
        >
          <label for="Registration" class="form-label"
            ><strong>Registration Number</strong></label
          >
          <input
            ...
            class="form-control"
            id="Registration"
            v-model="cocStudent.registration_no"
          />
          <span class="error-msg mt-1">{{
            v$.cocStudent.registration_no.$errors[0]?.$message
          }}</span>
        </div>
        <div class="d-flex mt-4">
          <div class="me-4"><strong>Nature of Assessement Applied</strong></div>
  ...
  <div v-if="isCheck" class="editwraper d-flex">
    <div class="alertBox border rounded shadow-sm p-3 bg-white ms-auto me-auto">
      <div class="mt-5 d-flex">
        ...
        <button @click="closeDialog" class="btn closebtn ms-auto mt-5 me-5">
          Close
        </button>
      </div>
    </div>
      ...
      if (this.searchValue?.length) {
        ...
        try {
          ...
          var response = await apiClient.post(
            "api/check_student_for_coc/" + this.searchValue,
    ...
    async submitInternalData() {
      if (
        ...
        try {
          ...
          var response = await apiClient.post(
            "api/register_internal_student/" + cocStudent.id,
            cocStudent
          );
          ...
          if (response.status === 201) {
            ...
            this.resultNotifier =
              "You have Registered a student to COC succesfully";
            var previousCocs = this.$store.getters[
              "registrar/cocTakerStudents"
            ];
      ...
      if (!this.v$.$error) {
        ...
        try {
          ...
          if (response.status === 201) {
            ...
            this.resultNotifier =
              "You have Registered a student to COC succesfully";
            var previousCocStudents = this.$store.getters[
              "registrar/cocTakerStudents"
            ];
            previousCocStudents.data.push(response.data);
            this.$store.commit(
              "registrar/setCocTakerStudents",
              previousCocStudents
            );
            this.v$.$reset();
          } else if (response.status === 200) {
